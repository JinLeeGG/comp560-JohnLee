...
Held-Out-Position Generalization in Micro-Transformers}
...
on a micro-transformer (under one million parameters) trained from scratch on a
relative-order task. Given a length-20 string containing one \texttt{X} and one
\texttt{Y}, the model decides whether \texttt{X} precedes \texttt{Y}. We train with both symbols in the first half of
the string and test with both in the second half, and we compare five positional
encodings under an otherwise identical model. All five learn the trained
...
encodings even fail differently, which we return to in the analysis.
...
with more seeds, testing other position splits (even/odd and distance-based
hold-outs), shrinking the model toward the smallest one that still solves the task,
and using mechanistic analysis to test the held-out-embedding explanation directly.
...
An AI assistant (Claude) was used substantially throughout this project. It helped
write the code (the positional-encoding engine, the data-preparation and evaluation
scripts, and the plotting code), ran the ten-seed sweep, aggregated the results,
and produced the figures. It also drafted the text of this report, which the author
then revised sentence by sentence. The author directed the research: choosing which
experiment to report, framing the questions, and reading the cited work personally
to verify the Related Work. The author reviewed every claim and number against the
experiment logs, caught and corrected errors in the drafts, and takes full
responsibility for the accuracy and originality of the final report.
