...
Length generalization for transformer models, training on short sequences and
testing on longer ones, has been studied extensively. But a transformer can also fail without any change
...
which encodings generalize. Most strikingly, rotary encoding generalizes well
here even though it is reported to generalize poorly for length. In our
fixed-length setting it behaves like the other relative encodings, but for length
generalization it behaves like the absolute ones. This reversal is direct evidence
that fixed-length position generalization is a distinct phenomenon from length
generalization.
\end{abstract}
...
then test on examples where it falls in the second half, without changing
...
and two absolute ones. The three relative ones are no positional encoding
(NoPE)~\citep{kazemnejad2023impact}, rotary (RoPE)~\citep{su2021roformer}, and a
T5-style relative bias (T5)~\citep{raffel2020t5}; the two absolute ones are a
learned positional encoding~\citep{radford2018improving} and a sinusoidal
positional encoding~\citep{vaswani2017attention}.
...
is the ten digits, \texttt{X}, \texttt{Y}, and the separator ``\texttt{:}''; the
two labels \texttt{T} and \texttt{F} appear only as the output token.

\subsection{The position split}\label{subsec:split}
...
input stream. The code for the model, data generation, and evaluation is available
at \url{https://github.com/JinLeeGG/comp560-JohnLee}.
...
\begin{itemize}
  ...
  \item \textbf{Absolute.} \emph{Learned}~\citep{radford2018improving}: a
  trainable position embedding is added to the token embedding at each index.
  \emph{Sinusoidal}: the fixed sine/cosine table of \citet{vaswani2017attention}
  is added instead.
\end{itemize}
...
mean, so the separation is robust. Sinusoidal in particular sits well below chance
rather than merely failing to transfer, a stronger failure we take up in
Section~\ref{subsec:twofail}.
...
seeds overlaid, and shows that the family gap holds on every seed while the
encodings differ in stability. Among the relative encodings RoPE is the least
stable, dropping to 79.6\% on one seed while NoPE and the T5 bias never fall below
92.9\%. The two absolute encodings fail differently: sinusoidal is below chance on
every seed, whereas learned is bimodal, ranging from 50\% up to 100\%, so its mean
is a value no single seed produced. Full per-seed numbers are in
...
\begin{figure}[t]
  ...
  sinusoidal sits below chance. RoPE's one low seed (79.6\%) is labeled. Because
  every encoding reaches 100\% validation accuracy, each gap below it is a
  generalization failure, not a learning failure.}
  \label{fig:heldout}
\end{figure}

Figure~\ref{fig:perpos} breaks accuracy down by the positions of \texttt{X} and
\texttt{Y}. The relative encodings are correct across the whole grid; the absolute
encodings are correct in the trained block (top-left) but fail in the held-out
block (bottom-right), where learned is uneven and sinusoidal falls well below
chance.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{per_position_multiseed.png}
  \caption{Per-position accuracy for each encoding (ten-seed mean); the vertical
  axis is \texttt{X}'s position and the horizontal is \texttt{Y}'s. The diagonal
  ($x=y$) is excluded because the two symbols never share a cell. Blue = correct,
  gray = chance, red = below chance. Each cell is one ordered position pair, so the
  grid covers every pair of positions. The top-left block has both symbols in 0--9 (trained)
  and the bottom-right both in 10--19 (held-out); the two off-diagonal blocks, with
  one symbol in each half, are shown only for context.
  In sinusoidal's held-out block, the split into a correct and an incorrect triangle
  reflects the model largely predicting a single label: it gives the same answer
  regardless of position, so it is correct only where that answer happens to be
  right, not because it reads the positions.}
  \label{fig:perpos}
\end{figure}
...
\subsection{Two ways for an absolute encoding to fail}\label{subsec:twofail}
The two absolute encodings fail in different ways, and
...
% ============================================================================
\appendix
\section{Per-seed held-out accuracy}
\label{app:perseed}
\renewcommand{\thetable}{A\arabic{table}}
\setcounter{table}{0}

Table~\ref{tab:perseed} gives the held-out accuracy of every encoding on each of
the ten seeds, the per-seed data behind Figure~\ref{fig:heldout} and the means in
Table~\ref{tab:main}.
