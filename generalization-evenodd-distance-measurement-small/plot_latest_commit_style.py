...
X/Y even/odd distance measurement sanity experiment.
...
CHANCE = 50.0
HALF = LENGTH // 2
D = 0            # unused for even/odd-distance bar/heatmap plots; keeps copied helper code parsable
...
def plot_bars(split, out_dir, methods_filter=None, suffix=''):
    ...
    ax.set_title(f'X/Y even/odd distance measurement — held-out generalization ({split} split)\n'
    ...
    ax.set_xlabel('')
    ax.set_title(f'X/Y even/odd distance measurement — per-class held-out accuracy ({split} split)\n'
...
def plot_per_position(split, out_dir, methods_filter=None, suffix=''):
    ...
    for ax in flat_axes[len(methods):]:
        ax.axis('off')
    fig.suptitle(f'X/Y even/odd distance measurement — per-position diagnostic sweep ({split} split, pooled over seeds)\n'
