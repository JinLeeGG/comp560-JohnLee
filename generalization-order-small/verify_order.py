...
For each trial we draw a single O-filled body and a single pair of positions
...
Usage (from generalization-order-small/, after training a checkpoint):
...
length = meta.get('length', length)
filler = meta.get('filler', 'O')
...
def predict(body):
    ...
    with torch.no_grad():
        logits = model.classify(ids, readout=ckpt.get('readout', 'mean_body'))
    class_names = ckpt.get('class_names', ['F', 'T'])
    return class_names[logits[0].argmax().item()]
...
for _ in range(n_trials):
    ...
    fillers = [filler for _ in range(length)]
