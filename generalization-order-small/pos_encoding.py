...
class T5RelativeBias(PositionalEncoding):
    """Simplified T5-style relative position bias.

    This tiny experiment deliberately does NOT use T5's logarithmic distance buckets.
    Every relative offset in the block gets its own learned bucket:

        bucket = key_index - query_index + (block_size - 1)

    That gives one independent scalar per (head, relative offset), which is the
    advisor's "individual bucket for every offset" version.
    """

    def __init__(self, n_head, block_size, causal=True):
        super().__init__()
        self.n_head = n_head
        self.block_size = block_size
        self.causal = causal
        self.offset = block_size - 1
    ...
    def attention_bias(self, T, device):
        ...
        buckets = rel + self.offset                                                  # (T, T), exact offset buckets
