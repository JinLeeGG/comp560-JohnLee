...
input_mode = ''         # if empty, read from checkpoint
# ----------------------------------------------------------------------
...
def predict_batch(bodies):
    ...
    for i in range(0, len(ids), 512):
        logits = model.classify(ids[i:i + 512], readout=readout, input_mode=input_mode)
        preds.extend(class_names[t] for t in logits.argmax(dim=-1).tolist())
    return preds
