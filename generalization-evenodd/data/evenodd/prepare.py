"""
Even/odd-separation task data generator (distance task #1).
...
20-char string, sees ':', and must output a SINGLE token:
    ...
    LABEL MAPPING (fixed -- never flip this):
        T  if  DISTANCE is EVEN
        F  if  DISTANCE is ODD

    482X5017X64019285746:F   <- X at 3, X at 8  -> distance 5 (odd)  -> F
    482X50178X4019285746:T   <- X at 3, X at 9  -> distance 6 (even) -> T

Why two identical X's (not X and Y): even/odd separation depends only on the DISTANCE
between the targets, not on their order, so a second distinct symbol would add an
irrelevant order cue. Two X's isolate distance cleanly. This is the contrast with the
relative-order task (which used X and Y because order mattered there).
...
distribution shift is over WHERE the two X's may appear, controlled by SPLIT:
    ...
    SPLIT='half'   train/val : BOTH X's in the FIRST half (0..9)
                   ...
                   Having learned to judge even/odd distance in the first half, does the
                   model still do it in unseen second-half positions? The point of the
                   task: NoPE ('none') is expected to FAIL here -- it can read ORDER from
                   the causal mask but cannot COUNT exact distance -- whereas on relative
                   order NoPE generalized. Distance-aware (rope/t5) and absolute methods
                   (learned/sinusoidal, which can subtract positions) may hold up better.

Distance balance (required for the accuracy-vs-separation graph): within the held-out
second half (positions 10..19) distances range 1..9, and larger distances have far fewer
position pairs (distance 9 = only the single pair {10,19}). Sampling positions uniformly
would starve the large distances. So the held-out TEST pool is built by sampling a
roughly-equal number of examples PER DISTANCE instead (every even distance gets the same
count, every odd distance gets the same count, chosen so T/F stays exactly 50/50). The
per-distance counts are printed below so coverage at each distance is verifiable.
...
def label_for(lo, hi):
    """The fixed label rule: T iff the distance |hi-lo| is even."""
    return 'T' if (hi - lo) % 2 == 0 else 'F'


def positions_for_label(allowed, label):
    """Pick two distinct positions from `allowed` whose distance parity matches `label`
    (T=even, F=odd). Rejection sampling -- about half of all pairs have each parity, so
    this terminates quickly. Returns sorted (lo, hi)."""
    while True:
...
def make_balanced_pool(n, role):
    """Build n examples, class-balanced 50/50 between T (even distance) and F (odd).
    Distances within each class are whatever rejection sampling yields -- training WANTS
    varied distances; only the held-out test pool needs explicit per-distance balance."""
...
def make_distance_balanced_test(n):
    """Held-out test pool with both X's in the second half (SPLIT='half'), sampling a
    roughly-equal number per DISTANCE and exactly 50/50 T/F.

    A contiguous region of size R has distances 1..R-1. Even distances -> T, odd -> F.
    Give every even distance the same count and every odd distance the same count, chosen
    so the two classes are exactly balanced (T_total == F_total == n//2). With R=10 and
    n=2000: distances 2,4,6,8 -> 250 each (T=1000); distances 1,3,5,7,9 -> 200 each
    (F=1000)."""
    allowed = allowed_positions('test')
    R = len(allowed)
    dists = list(range(1, R))                          # 1..R-1
    even = [d for d in dists if d % 2 == 0]            # -> T
    odd = [d for d in dists if d % 2 == 1]             # -> F
    half_n = n // 2
    per_even = half_n // len(even)
    per_odd = half_n // len(odd)
    examples, per_distance = [], {}
    for d in dists:
        cnt = per_even if d % 2 == 0 else per_odd
        label = 'T' if d % 2 == 0 else 'F'
        for _ in range(cnt):
...
def check_labels(examples):
    """Assert |pos2 - pos1| is EVEN iff label is 'T' for EVERY example, and that exactly
    two X's are present. Position determines the answer, so a placement/label bug must be
...
print("=== Even/odd-separation data prepared ===")
...
print(f"label rule    : T iff |pos2-pos1| is EVEN, else F")
...
for name, ex, role in [('train', train_examples, 'train'),
    ...
    print(f"{name:5s}: {len(ex):>6d} examples  | T(even)={t} F(odd)={fa}  ({t / len(ex):.1%} T)")
print("label-correctness assertion passed for all pools  (|pos2-pos1| even iff T)")
...
if test_per_distance is not None:
    ...
    for d in sorted(test_per_distance):
        lab = 'T even' if d % 2 == 0 else 'F odd '
...
def show(examples, k=6):
    for e in examples[:k]:
        body, label = e.split(':')
        lo, hi = x_positions(body)
        print(f"  X@{lo:>2} X@{hi:>2}  dist={hi - lo:>2} -> {label}   {e}")
