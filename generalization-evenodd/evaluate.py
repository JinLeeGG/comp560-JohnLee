"""
Evaluation script for the even/odd-separation task (from-scratch micro-transformer engine).

Reads the held-out test examples (data/evenodd/test.txt), feeds each '<20 chars>:' prompt
to the model, reads the argmax at the answer position (the token after ':'), and checks it
against the gold T/F label.

    LABEL MAPPING (must match prepare.py):  T  iff  |pos2 - pos1| is EVEN   (distance even).

Because the task is binary, chance is 50%. We report PER-CLASS accuracy (T = even distance,
F = odd distance), not just overall: a model that always outputs one label scores 50%
overall, and per-class accuracy is what makes that failure mode visible. For errors we
print where the two X's landed and the distance between them.
...
This script also LOGS results so figures regenerate themselves as methods/seeds accumulate:
  ...
  - predictions.csv : one row per example of a DIAGNOSTIC SWEEP that spans ALL position
                      ...
                      and their distance, so plot.py can show the per-position "cliff" AND
...
Usage (from generalization-evenodd/):
    ...
    ../venv/bin/python evaluate.py config/basic.py --test_file=data/evenodd/test.txt
"""
import os
import sys
import csv
...
counts = {'T': [0, 0], 'F': [0, 0]}   # label -> [correct, total]  (T=even dist, F=odd dist)
nonbinary = 0
errors = []
...
def build_sweep(per_pair, length, sweep_seed=0):
    ...
    and held-out regions) and is class-balanced by construction (even distance -> T, odd
    -> F). Deterministic, so the per-position heatmap and separation curve are comparable
    across runs/methods. The two targets are identical, so only the unordered pair matters
    -- we log x1_pos < x2_pos and their distance."""
    rng = random.Random(sweep_seed)
    out = []
    for x1 in range(length):
        for x2 in range(x1 + 1, length):
            dist = x2 - x1
            label = 'T' if dist % 2 == 0 else 'F'
