"""
Training loop for the from-scratch micro-transformer (even/odd-separation task).

Same engine and structure as generalization-order / generalization-detect; only the data
dir and task naming differ. Two deliberate departures from the nanoGPT flat-stream
baseline (see the spec):

  1. Per-example batching. Each row is exactly one example -- "<20 chars>:<label>" --
     ...
     for even/odd separation, it does: the answer is the parity of the X-X distance.)

  2. Answer-token-only loss. Cross-entropy is computed ONLY at the answer position
     (the token after ':'), not over every token. The 18 filler digits are random and
     unpredictable, so supervising them is wasted signal; supervising just the answer
     makes the loss meaningful and training cleaner.
...
def load_examples(name):
    ...
    Each example is "<body>:<label>"; we feed the body+':' as input and supervise the
...
model_args = dict(vocab_size=vocab_size, block_size=block_size, n_layer=n_layer,
                  ...
                  pos_type=pos_type, causal=causal)
