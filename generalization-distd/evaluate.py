"""
Evaluation script for the dist>=D threshold task (from-scratch micro-transformer engine).

Reads the held-out test examples (data/distd/test.txt), feeds each '<20 chars>:' prompt to
the model, reads the argmax at the answer position (the token after ':'), and checks it
against the gold T/F label.

    LABEL MAPPING (must match prepare.py):  T  iff  |pos2 - pos1| >= D   (the two X's are
    FAR apart, distance >= 5); F otherwise (NEAR, distance <= 4). D is read from meta.pkl.

Because the task is binary, chance is 50%. We report PER-CLASS accuracy (T = far, F = near),
not just overall: a model that always outputs one label scores 50% overall, and per-class
accuracy is what makes that failure mode visible. For errors we print where the two X's
landed and the distance between them.
...
This script also LOGS results so figures regenerate themselves as methods/seeds accumulate:
  ...
  - predictions.csv : one row per example of a DIAGNOSTIC SWEEP that spans ALL position
                      pairs (train region AND held-out region), recording both X positions
                      and their distance, so plot.py can show the per-position pattern AND
                      the accuracy-vs-separation curve. The sweep is generated here,
                      deterministically, independent of the data split, so figures are
                      comparable across runs.

    Note on the per-position heatmap: on the 'half' split, "dist>=D" correlates with "X's at
    opposite ENDS of 10..19", so a model could win by a POSITION shortcut instead of reading
    distance. Genuine distance -> BANDS parallel to the diagonal in the heatmap; a shortcut
    -> BLOCKS. plot.py draws the heatmap; read it before claiming the model reads distance.

Usage (from generalization-distd/):
    ../venv/bin/python evaluate.py config/basic.py --pos_type=none
    ../venv/bin/python evaluate.py config/basic.py --test_file=data/distd/test.txt
"""
import os
import sys
import csv
...
sweep_per_pair = 20     # examples per (x1_pos, x2_pos) cell in the diagnostic sweep
...
# load meta (vocab + split info + threshold D, written by prepare.py)
with open(os.path.join(data_dir, 'meta.pkl'), 'rb') as f:
    meta = pickle.load(f)
stoi, itos = meta['stoi'], meta['itos']
split = meta.get('split', 'unknown')
split_detail = meta.get('split_detail', '')
D = meta.get('threshold', 5)              # the dist>=D threshold; single-sourced from the data
...
def x_positions(body):
    """The two X positions (sorted) in a body string."""
...
counts = {'T': [0, 0], 'F': [0, 0]}   # label -> [correct, total]  (T=far dist>=D, F=near dist<D)
nonbinary = 0
errors = []
...
# report
print("=== dist>=D threshold Evaluation ===")
print(f"test_file: {test_file}")
print(f"pos_type : {pos_type} | causal: {causal} | t5_bias_mode: {t5_bias_mode} | split: {split} ({split_detail}) | D={D} | seed: {train_seed}")
print(f"val acc  : {val_acc:.2%}" if val_acc is not None else "val acc  : (not in ckpt)")
print(f"examples : {total}   (chance = 50%)")
if tT:
    print(f"T (far,  dist>={D}): {cT}/{tT} = {heldout_T_acc:.2%}")
if tF:
    print(f"F (near, dist<{D}) : {cF}/{tF} = {heldout_F_acc:.2%}")
if total:
    print(f"OVERALL           : {correct}/{total} = {heldout_acc:.2%}")
if nonbinary:
    print(f"(note: {nonbinary} predictions were neither T nor F)")
if errors and show_errors:
    print(f"\nfirst {min(show_errors, len(errors))} errors (X positions, distance):")
    for body, gold, pred in errors[:show_errors]:
        lo, hi = x_positions(body)
        print(f"  X@{lo:>2} X@{hi:>2}  dist={hi - lo:>2}  gold={gold} pred={pred}   {body}")
...
def build_sweep(per_pair, length, threshold, sweep_seed=0):
    """Diagnostic sweep: for EVERY unordered position pair (x1 < x2), `per_pair` random
    bodies with an X at x1 and an X at x2. Spans the whole position range (both the train
    and held-out regions) and is class-balanced by construction only if pairs were uniform
    -- here it is NOT, since far pairs outnumber near pairs; the sweep is for the
    per-position heatmap and the per-distance curve, both of which condition on (x1,x2) or
    distance, so overall class balance does not matter. Deterministic, so figures are
    comparable across runs/methods. The two targets are identical, so only the unordered
    pair matters -- we log x1_pos < x2_pos and their distance. Label: T iff dist >= threshold."""
    rng = random.Random(sweep_seed)
    out = []
    for x1 in range(length):
        for x2 in range(x1 + 1, length):
            dist = x2 - x1
            label = 'T' if dist >= threshold else 'F'
            for _ in range(per_pair):
                chars = [rng.choice('0123456789') for _ in range(length)]
                chars[x1], chars[x2] = 'X', 'X'
                out.append((''.join(chars), x1, x2, dist, label))
    return out
...
if log_results:
    ...
    # 2) per-example diagnostic sweep -> predictions.csv (distance logged explicitly for
    #    the accuracy-vs-separation graph, though it is derivable from x1_pos/x2_pos)
    sweep = build_sweep(sweep_per_pair, LENGTH, D)
    sweep_preds = predict_batch([b for b, _, _, _, _ in sweep])
    rows = [[pos_type, split, train_seed, x1, x2, dist, gold, pred, int(pred == gold)]
            for (_, x1, x2, dist, gold), pred in zip(sweep, sweep_preds)]
    append_csv(predictions_csv,
               ['pos_type', 'split', 'seed', 'x1_pos', 'x2_pos', 'distance',
                'gold', 'pred', 'correct'],
               rows)

    sweep_acc = sum(r[8] for r in rows) / len(rows)
