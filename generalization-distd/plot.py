...
def plot_bars(split, out_dir, methods_filter=None, suffix=''):
    ...
    ax.set_title(f'dist>={D} threshold — held-out generalization ({split} split)\n'
...
def plot_per_position(split, out_dir, methods_filter=None, suffix=''):
    ...
    for ax in flat_axes[len(methods):]:
        ax.axis('off')
    fig.suptitle(f'dist>={D} threshold — per-position diagnostic sweep ({split} split, pooled over seeds)\n'
                 f'cell = accuracy for X at row/col; bands parallel to diagonal suggest distance, '
                 f'blocks suggest position shortcut', fontsize=12)
