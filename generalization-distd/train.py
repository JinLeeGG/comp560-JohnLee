"""
Training loop for the from-scratch micro-transformer (dist>=D threshold task).

Same engine and structure as generalization-evenodd / generalization-order; only the data
...
baseline (see the spec):
  ...
  1. Per-example batching. Each row is exactly one example -- "<20 chars>:<label>" --
     ...
     for dist>=D, it does: the answer is whether the X-X distance reaches the threshold.)
